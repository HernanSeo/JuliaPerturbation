...
function smc_rwmh_neoclassical(initial_para, data, model, PAR, c)
    ...
    #par
    @inbounds Threads.@threads for i=1:npart
        #println(i)
        p0 = parasim[1,i,:]
        #println(size(p0))
...
function mutation_RWMH(data, p0, l0, lpost0, c, R, npara, phi_smc, nobs, i)
    ...
    # RW proposal
    @views px = p0 .+ (c*cholesky(Hermitian(R)).U'*randn(npara,1))'
...
function kf(y, TT, RQR, DD, ZZ, HH, t0)#, At) # this is a modification to the original code by Herbst where I input directly RQR instead of R & Q
    ...
    diferenz=0.1
    while diferenz>1e-25
        @views P_10 =TT_old*P_10_old*TT_old' + RQR_old
        diferenz = maximum(abs.(P_10-P_10_old))
        @views RQR_old=TT_old*RQR_old*TT_old' + RQR_old
        @views TT_old = TT_old * TT_old
        @views P_10_old=P_10
    end    #while diferenz
    Pt=P_10
    loglh = 0.0
    ...
    @inbounds for i in 1:nobs
        # yhat = ZZ*At .+ DD
        @views mul!(yaux,ZZ,At)
        @views yhat = yaux + DD
        if isnan(sum(yhat))
            println(i)
        end
        nut = (y[i, :] - yhat)'
        @views mul!(PtZZ,Pt,ZZ')
        @views mul!(ZPZ,ZZ,PtZZ)
        @views Ft = ZPZ + HH #ZZ*Pt*ZZ' .+ HH
        Ft = 0.5*(Ft + Ft')
        @views dFt = det(Ft)
        @views iFt = inv(Ft)
        @views mul!(iFtnut,iFt,nut')
        if isnan(sum(nut*iFtnut))
            println(i)
        end
        @views loglhvec[i] = - 0.5*log(dFt) - (0.5*nut*iFtnut)[1]
        @views mul!(TTPt,TT,Pt)
        @views mul!(Kt,TTPt,ZZ')
        @views mul!(TAt,TT,At)
        @views mul!(KiF,Kt,iFtnut)
        @views At = TAt + KiF
        @views mul!(TTPtTT,TTPt,TT')
        @views mul!(KFK,Kt,(Ft\Kt'))
        @views Pt = TTPtTT - KFK + RQR
    end
